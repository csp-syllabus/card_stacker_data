...
class DoubleSlider extends display.Container {
  constructor(options) {
    super();
    this.dragging1 = 0;
    this.dragging2 = 0;
    ...
    const bg = typeof options.bg === "string" ? new sprite.Sprite(core.Texture.from(options.bg)) : options.bg;
    this.bg = new display.Container();
    this.bg.addChild(bg);
    this.addChild(this.bg);
    if (options.fill) {
      const fill = typeof options.fill === "string" ? new sprite.Sprite(core.Texture.from(options.fill)) : options.fill;
    ...
    const slider1 = typeof options.slider1 === "string" ? new sprite.Sprite(core.Texture.from(options.slider1)) : options.slider1;
    if (slider1 instanceof sprite.Sprite) {
      slider1.anchor.set(0.5);
    }
    slider1.x = slider1.width / 2;
    this.slider1 = new display.Container();
    this.slider1.addChild(slider1);
    this.slider1.y = this.bg.height / 2;
    const slider2 = typeof options.slider2 === "string" ? new sprite.Sprite(core.Texture.from(options.slider2)) : options.slider2;
    if (slider2 instanceof sprite.Sprite) {
      slider2.anchor.set(0.5);
    }
    slider2.x = slider2.width / 2;
    this.slider2 = new display.Container();
    this.slider2.addChild(slider2);
    this.slider2.y = this.bg.height / 2;
    this.addChild(this.slider2, this.slider1);
    if (options.showValue) {
      this.slider1Text = new text.Text(
        "",
    ...
    if (options.showValue) {
      this.slider2Text = new text.Text(
        "",
        options.valueTextStyle || { fill: 16777215 }
      );
  ...
  makeScrollable() {
    ...
    hitbox.removeHitBox(this.fill, this.slider1Text, this.slider2Text);
  }
  onSetByClick(event) {
  ...
  onDragStart1(event) {
    ...
    obj.dragObjStart = new core.Point();
    obj.dragObjStart.copyFrom(obj.position);
    obj.dragGlobalStart = new core.Point();
